...
async def detect_issues(lines: list[str]) -> list[dict]:
    ...
    system_prompt = (
        ...
        "  critical — runtime failure that is actively breaking the bot RIGHT NOW:\n"
        "             gateway crash, LLM call failing with unrecoverable error, process unresponsive.\n"
        "             Do NOT use critical for: slow responses, context loss, architecture observations,\n"
        "             unexpected-but-handled errors, or anything a human cannot fix immediately.\n"
        "  warning  — degraded behaviour that needs attention but is not an emergency:\n"
        "             high token usage, repeated soft errors, unexpected but recovered failures.\n"
        "  info     — normal operational noise: debug output, heartbeat, routine tool calls.\n"
        "\n"
        "fix_type rules:\n"
